...
function pullPack() {
  ...
  if (results.every(id => getCard(id).rarity === "N")) {
    const better = pool.filter(c => c.rarity !== "N");
    results[results.length - 1] = better[Math.floor(Math.random() * better.length)].id;
  }
  for (const id of results) addToCollection(id, 1);
  return results;
}
...
function renderHand() {
  ...
  for (const cardId of me.hand) {
    ...
    div.className = "handCard " + (playable ? "canPlay" : "cannotPlay") + (isHoloRarity(card.rarity) ? " holoFoil" : "");
...
function refreshMenu() {
  ...
  document.getElementById("deckStatus").textContent =
    (deck ? "カスタムデッキ(30枚)で対戦します" : "初期デッキ(N24種ベース・30枚)で対戦します") + ` / 所持PT: ${loadCurrency()}`;
}
...
function doGachaPull() {
  ...
  const sorted = [...results].sort((a, b) => Object.keys(RARITY_RATES).indexOf(getCard(a).rarity) - Object.keys(RARITY_RATES).indexOf(getCard(b).rarity));
  sorted.forEach((cardId, i) => {
    const card = getCard(cardId);
    const div = document.createElement("div");
    div.className = "gachaCard" + (isHoloRarity(card.rarity) ? " holoFoil" : "");
    div.style.animationDelay = `${i * 0.25}s`;
    div.innerHTML = `<div class="rarity ${card.rarity}">${card.rarity}</div><div class="grname">${card.name}</div><div class="pmeta">${card.type}</div>`;
    box.appendChild(div);
    if (["UR", "XR", "SEC"].includes(card.rarity)) {
      setTimeout(() => flashBanner(`${card.rarity} GET!`, "awaken"), i * 250 + 300);
    }
  });
  log(`パックを開封: ${results.map(id => cardName(id)).join(" / ")}`);
}
...
function renderDeckScreen() {
  ...
  for (const card of deckablePool()) {
    ...
    const div = document.createElement("div"); div.className = "poolCard" + (isHoloRarity(card.rarity) ? " holoFoil" : "");
    div.innerHTML = `<div class="rarity ${card.rarity}">${card.rarity}</div><div class="pname">${card.name}</div><div class="pmeta">${card.type} / C${card.id === "VC-056" ? "X" : card.cost}</div><div class="ownedCount">所持: ${owned}</div>
...
function openCollectionScreen() {
  ...
  for (const card of CARD_POOL) {
    const owned = ownedCount(card.id);
    const div = document.createElement("div"); div.className = "poolCard" + (owned === 0 ? " locked" : "") + (owned > 0 && isHoloRarity(card.rarity) ? " holoFoil" : "");
    div.innerHTML = `<div class="rarity ${card.rarity}">${card.rarity}</div><div class="pname">${owned > 0 ? card.name : "???"}</div><div class="pmeta">${owned > 0 ? `${card.type} / C${card.id === "VC-056" ? "X" : card.cost}` : "未所持"}</div>${owned > 0 ? `<div class="ownedCount">所持: ${owned}</div>` : ""}`;
    grid.appendChild(div);
  }
